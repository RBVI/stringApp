...
public class PubMedQueryPanel extends JPanel { 
	...
	private void init() {
		...
		add(limitSlider, c.down().expandBoth().insets(5,5,0,5));
	...
	JPanel createSearchPanel() {
		...
		queryPanel.setPreferredSize(new Dimension(600,400));
		EasyGBC c = new EasyGBC();
	...
	void importNetwork(List<TextMiningResult> tmResults) {
		...
		TaskFactory factory = new ImportNetworkTaskFactory(stringNetwork, species.toString(), 
		                                                   species.getTaxId(), confidence, 0, stringIds,
																											 queryTermMap);
		TaskIterator importTaskIterator = factory.createTaskIterator();
		importTaskIterator.append(new AddTextMiningResultsTask(stringNetwork, tmResults));
		cancel();
		manager.execute(importTaskIterator);
	}
    ...
    public void actionPerformed(ActionEvent e) {
			...
			manager.execute(new TaskIterator(new GetStringIDsFromPubmedTask(stringNetwork, taxon, limitSlider.getValue(), query)),this);
		}
		...
		public void taskFinished(ObservableTask task) {
			...
			GetStringIDsFromPubmedTask annTask = (GetStringIDsFromPubmedTask)task;

			final List<TextMiningResult> results = annTask.getTextMiningResults();
